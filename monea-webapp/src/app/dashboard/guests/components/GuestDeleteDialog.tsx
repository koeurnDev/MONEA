...
interface GuestDeleteDialogProps {
    deleteGuest: { id: string, name: string } | null;
    onOpenChange: (open: boolean) => void;
    onConfirm: () => void;
}

export function GuestDeleteDialog({ deleteGuest, onOpenChange, onConfirm }: GuestDeleteDialogProps) {
    const { t } = useTranslation();

    return (
        <Dialog open={!!deleteGuest} onOpenChange={onOpenChange}>
            <DialogContent className="w-[92vw] max-w-[400px] rounded-[2rem] border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.4)] bg-card/95 backdrop-blur-3xl supports-[backdrop-filter]:bg-card/80 p-6 md:p-10 overflow-hidden">
                ...
                <DialogHeader className="text-center relative z-10">
                    ...
                    <DialogDescription className="font-kantumruy text-sm md:text-base text-muted-foreground/70 leading-relaxed px-2">
                        {t("guests.delete.description")} <strong>{deleteGuest?.name}</strong>?
                    </DialogDescription>
                </DialogHeader>
