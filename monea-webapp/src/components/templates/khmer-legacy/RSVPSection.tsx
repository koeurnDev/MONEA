        ...
        try {
            const res = await fetch('/api/wedding/rsvp', {
                ...
                body: JSON.stringify({
                    ...
                    childrenCount: 0,
                    rsvpNotes: `Name: ${inputName || "Guest"}\nNotes: ${inputNotes}`
                })
            });
                ...
                if (status === 'CONFIRMED') {
                    ...
                    const frame = () => {
                        confetti({
                            ...
                            spread: 55,
                            origin: { x: 0 },
                            colors: ['#D4AF37', '#F3E5AB', '#ffffff'] // Gold and white to match theme
                        });
                        confetti({
                            particleCount: 5,
                            angle: 120,
            ...
            <section className="py-16 md:py-32 bg-white text-center px-6 relative overflow-hidden">
                ...
                >
                    ...
                    <div className="flex flex-col items-center gap-6 pt-6">
                        ...
                        {status === 'CONFIRMED' && wedding.date && (
                            <m.a
                                href={`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent((wedding.groomName || '') + ' & ' + (wedding.brideName || ''))}&dates=${new Date(wedding.date).toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z'}/${new Date(wedding.date).toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z'}&details=${encodeURIComponent('We look forward to seeing you at our wedding!')}&location=${encodeURIComponent(wedding.location || 'Phnom Penh')}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                                className="mt-4 flex items-center gap-3 px-6 py-3 bg-white border border-gold-main/20 rounded-full text-gold-main font-khmer-content font-black text-sm hover:bg-gold-main/5 transition-colors shadow-sm"
                            >
                                <Calendar size={18} />
                                បញ្ចូលក្នុងប្រតិទិន (Add to Calendar)
                            </m.a>
                        )}
                    </div>
                </m.div>
        ...
        >
            ...
            <div className="max-w-3xl mx-auto relative z-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-12 bg-white/60 backdrop-blur-2xl p-8 md:p-12 rounded-[3rem] border border-white/40 shadow-[0_40px_100px_rgba(0,0,0,0.08)] relative overflow-hidden group">
                            ...
                            >
                                {!guestName && (
                                    <div className="flex flex-col gap-2 max-w-md mx-auto w-full">
                                        ...
                                        <input 
                                            ...
                                            required
                                            className="w-full h-14 px-6 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-gold-main focus:ring-2 focus:ring-gold-main/20 transition-all font-khmer-content"
                                ...
                                <div className="flex flex-col gap-2 max-w-md mx-auto w-full">
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full p-4 rounded-2xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-gold-main focus:ring-2 focus:ring-gold-main/20 transition-all font-khmer-content resize-none h-24"
                                        placeholder="បញ្ជាក់ពីតម្រូវការអាហារ ឬពាក្យជូនពរ..."
                                    />
                                </div>
                            </m.div>
                        )}
                    ...
                    <m.button
                        type="submit"
                        disabled={!status || (!guestName && !inputName.trim()) || isSubmitting}
                        whileHover={{ scale: 1.02, y: -4 }}
                        whileTap={{ scale: 0.96 }}
