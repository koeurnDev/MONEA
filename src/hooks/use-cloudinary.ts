...
export function useCloudinary(options: UseCloudinaryOptions = {}) {
    ...
    const uploadFiles = async (files: File[]) => {
        ...
        try {
            // Get signature first (we use one signature for all uploads if possible, 
            // but Cloudinary signatures usually include timestamp, so it might expire if upload takes too long.
            // However, usually we sign per file or just sign once if the simplified signing is used.
            // The original code signed PER file. We will keep that behavior to be safe and consistent.
