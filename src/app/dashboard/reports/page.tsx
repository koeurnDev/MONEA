...
import { Download, Users, DollarSign, Gift, MessageSquare, CheckCircle, Activity } from "lucide-react";
...
export default function ReportsPage() {
    ...
    const exportFinancialReport = async () => {
        const safeGifts = Array.isArray(gifts) ? gifts : [];
        const BOM = "\uFEFF";
        const headers = ["ឈ្មោះភ្ញៀវ", "ទីតាំង", "ចំនួន", "រូបិយប័ណ្ណ", "វិធីសាស្ត្រ", "កាលបរិច្ឆេទ"];
        const rows = safeGifts.map((g: any) => [
            ...
            g.currency,
            g.method || "Cash",
            `${new Date(g.createdAt).toISOString().split('T')[0]} ${new Date(g.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}`
        ]);

        const csvContent = [headers, ...rows].map((e: any[]) => e.map(i => `"${i}"`).join(",")).join("\n");
    ...
    return (
        <div className="space-y-10 pb-10">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                {[
                    { label: "ភ្ញៀវសរុប", value: stats.totalGuests, sub: "Check-in: " + stats.checkInCount, icon: Users, color: "text-blue-600", bg: "bg-blue-50/50" },
                    { label: "សាច់ប្រាក់ដុល្លារ", value: "$" + stats.totalGiftsUsd.toLocaleString(), sub: "សរុបចំនួន " + stats.giftsCount + " នាក់", icon: DollarSign, color: "text-emerald-600", bg: "bg-emerald-50/50" },
                    { label: "សាច់ប្រាក់រៀល", value: stats.totalGiftsKhr.toLocaleString() + " ៛", sub: "សរុបចំនួន " + stats.giftsCount + " នាក់", icon: Gift, color: "text-amber-600", bg: "bg-amber-50/50" },
                    { label: "ពាក្យជូនពរ", value: stats.wishesCount, sub: "ពីភ្ញៀវកិត្តិយស", icon: MessageSquare, color: "text-pink-600", bg: "bg-pink-50/50" },
                ].map((stat, i) => (
                    <Card key={i} className="border-none shadow-sm hover:shadow-md transition-all rounded-[2rem] overflow-hidden group relative bg-white">
                        <CardContent className="p-8 pt-10">
                            ...
                            <div className="space-y-1">
                                <div className="text-3xl font-black text-slate-900 font-kantumruy">
                                    {loading ? "..." : stat.value}
                                </div>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{stat.label}</p>
                            </div>
                            <div className="mt-6 pt-4 border-t border-slate-50">
                                <span className="text-[10px] font-bold text-slate-500 font-kantumruy">{stat.sub}</span>
                            </div>
                        </CardContent>
                    </Card>
                ))}
            ...
            <div className="grid gap-10 lg:grid-cols-12">
                <div className="lg:col-span-12">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-lg font-bold text-slate-900 font-kantumruy">សកម្មភាព និងស្ថិតិលម្អិត</h3>
                    </div>
                </div>

                {/* Left Side: Activity Log */}
                <Card className="lg:col-span-7 border-none shadow-sm rounded-[2rem] bg-white overflow-hidden">
                    <CardHeader className="p-8 pb-4 border-b border-slate-50 flex flex-row items-center justify-between">
                        ...
                        <div className="px-3 py-1 rounded-full bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-widest border border-slate-100">
                            Live updates
                        </div>
                    </CardHeader>
                    <CardContent className="p-0">
                        {!Array.isArray(logs) || logs.length === 0 ? (
                            ...
                            <div className="divide-y divide-slate-50">
                                {logs.map((log: any) => (
                                    <div key={log.id} className="p-6 px-8 hover:bg-slate-50 transition-colors flex gap-6 items-start group">
                                        <div className={cn(
                                            "w-10 h-10 rounded-xl flex items-center justify-center transition-all",
                                            log.action === 'GIFT' ? 'bg-emerald-50 text-emerald-600' :
                                                log.action === 'CHECK_IN' ? 'bg-blue-50 text-blue-600' :
                                                    log.action === 'WISH' ? 'bg-pink-50 text-pink-600' : 'bg-slate-50 text-slate-600'
                                        )}>
                                        ...
                                        <div className="flex-1">
                                            <p className="font-bold text-slate-800 font-kantumruy leading-relaxed">{log.description}</p>
                ...
                <div className="lg:col-span-5 space-y-8">
                    <Card className="border-none shadow-sm rounded-[2rem] bg-white overflow-hidden">
                        <CardHeader className="p-8 pb-4 border-b border-slate-50">
                            <CardTitle className="text-lg font-bold text-slate-900 font-kantumruy">ស្ថិតិតាមទីតាំង</CardTitle>
                        </CardHeader>
                                    ...
                                    return sortedSources.map(([name, data]: [string, any]) => (
                                        <div key={name} className="space-y-3">
                                            <div className="flex justify-between items-end">
                                                <div>
                                                    <p className="text-sm font-black text-slate-900 font-kantumruy">{name}</p>
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{data.guests} នាក់</p>
                                                </div>
                                                <div className="text-right">
                                                    <p className="text-xs font-black text-emerald-600">${data.usd.toLocaleString()}</p>
                                                    <p className="text-[10px] font-bold text-slate-400 font-mono">{data.khr.toLocaleString()} ៛</p>
                                                </div>
                                            </div>
                    ...
                    <div className="grid gap-6">
                        <Button
                            ...
                            className="h-14 bg-white border border-slate-100 hover:bg-slate-50 text-slate-900 rounded-2xl shadow-sm hover:shadow transition-all group px-6"
                        >
                            <div className="flex items-center justify-between w-full">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="font-bold font-kantumruy">CSV (ចំណងដៃ)</span>
                                </div>
                                <span className="text-[10px] font-bold text-slate-300 tracking-widest uppercase">Export</span>
                            </div>
                        </Button>
                        ...
                        >
                            <div className="flex items-center justify-between w-full">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="font-bold font-kantumruy">CSV (បញ្ជីភ្ញៀវ)</span>
                                </div>
                                <span className="text-[10px] font-bold text-slate-300 tracking-widest uppercase">Export</span>
                            </div>
                        </Button>
