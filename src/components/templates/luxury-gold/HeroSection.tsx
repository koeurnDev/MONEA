...
export default function HeroSection({ wedding, labels, addToCalendarUrl }: HeroSectionProps) {
    ...
    return (
        <m.section style={{ opacity: opacityHero }} className="relative h-screen flex flex-col items-center justify-end pb-24 text-center overflow-hidden">
            ...
            <div className="relative z-20 p-6 w-full pt-32 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/60 to-transparent">
                <m.div {...fadeInUp}>
                    ...
                    <div className="relative inline-block mb-4 md:mb-8">
                        ...
                        <h1 className="text-3xl md:text-6xl text-wedding-title mb-1 md:mb-3 leading-relaxed drop-shadow-sm animate-shine">
                            {wedding.groomName}
                        </h1>
                        <span className="text-3xl md:text-5xl font-greatvibes text-[#D4AF37] block my-0.5 md:my-1">&</span>
                        <h1 className="text-3xl md:text-6xl text-wedding-title leading-relaxed drop-shadow-sm animate-shine">
                            {wedding.brideName}
                        </h1>
                    </div>
                    ...
                    <div className="flex flex-col gap-1 md:gap-2 font-khmer text-[#F3E5AB]/90">
                        <p className="text-base md:text-xl tracking-wide font-medium">
                            {new Date(wedding.date).toLocaleDateString('km-KH', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
                        </p>
                        <p className="text-[10px] md:text-sm opacity-80 font-light border-y border-[#D4AF37]/30 py-1 md:py-2 inline-block mx-auto px-4 md:px-6 mt-1 md:mt-2 tracking-wider">
