...
export default function MasterMaintenancePage() {
    ...
    const confirmCleanup = async () => {
        ...
        try {
            ...
            if (res.data) {
                setCleanResult(`Cleanup successful! Removed ${res.data.deletedCount} old logs.`);
            } else if (res.error) {
                setCleanResult(`Cleanup failed: ${res.error}`);
            }
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#0c0c14] p-8">
            <ConfirmModal
                open={cleanupConfirm}
            ...
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card className="border border-slate-200 dark:border-slate-800 shadow-sm rounded-[2.5rem] bg-white dark:bg-slate-900 p-8">
                            ...
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent dark:border-slate-800">
                                        <span className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest">{item.label}</span>
                                        <span className={cn(
                                            "font-black text-lg",
                                            item.alert ? "text-red-500" : "text-slate-900 dark:text-white"
                        ...
                        <div className="space-y-6">
                            <Card className="border border-slate-200 dark:border-slate-800 shadow-sm rounded-[2.5rem] bg-slate-900 p-8 text-white">
