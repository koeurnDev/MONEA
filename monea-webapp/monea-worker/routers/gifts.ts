...
giftsRouter.get('/stats', async (c) => {
    try {
        ...
        let weddingId = null;
        if ((user as any).type === "staff" || user.role === "STAFF") {
            weddingId = (user as any).weddingId;
        } else {
            const wedding = await prisma.wedding.findFirst({ where: { userId: user.userId || (user as any).id } });
...
giftsRouter.get('/', async (c) => {
    try {
        ...
        if (!weddingId) return c.json({ items: [], pagination: { total: 0, limit, offset, hasMore: false }, role: user.role });
...
giftsRouter.post('/', async (c) => {
    ...
    try {
        ...
        const weddingId = user.weddingId || (await prisma.wedding.findFirst({ where: { userId: user.userId || user.id } }))?.id;
        ...
        return c.json({ error: "Failed to save gift", details: error.message }, 500);
    }
});
