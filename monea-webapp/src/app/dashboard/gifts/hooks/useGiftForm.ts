    ...
    async function onSubmit(values: GiftFormValues, keepOpen: boolean) {
        ...
        try {
            ...
            if (res.error) {
                ...
                if (res.status === 429 || res.error.toLowerCase().includes("limit") || res.error.toLowerCase().includes("upgrade")) {
                    const limitErr = new Error(res.error);
                    (limitErr as any).isLimitError = true;
                    throw limitErr;
                }
