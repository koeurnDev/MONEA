...
const TEMPLATES = [
    { id: "modern-minimal", title: "ទាន់សម័យ (Modern Minimal)", categories: ['wedding'], bgClass: "bg-slate-100", textClass: "text-slate-800", image: "", isFree: false, comingSoon: false },
    { id: "anniversary-elegant", title: "ខួបប្រណីត (Elegant)", categories: ['anniversary'], bgClass: "bg-amber-50", textClass: "text-amber-800", image: "/assets/anniversary-elegant/anniversary-elegant-bg.webp", isFree: true, comingSoon: false },
];
                ...
                <button
                    ...
                    className={clsx(
                        ...
                        wedding.eventType === 'wedding' 
                            ...
                            : "text-slate-400 dark:text-white/40 hover:bg-white dark:hover:bg-white/10 hover:text-slate-700 dark:hover:text-white"
                    )}
                >
                ...
                <div className="grid grid-cols-1 gap-4">
                    {TEMPLATES.filter(t => t.categories.includes(wedding.eventType || 'wedding')).map((tmpl, idx) => (
                        <m.div
                            ...
                            className={clsx(
                                "rounded-[2rem] p-4 transition-all duration-500 relative overflow-hidden border-2",
                                tmpl.comingSoon ? "cursor-not-allowed opacity-60 bg-slate-50 dark:bg-white/5 border-transparent grayscale" : "cursor-pointer group",
                                (!tmpl.comingSoon && (wedding.templateId === tmpl.id || (!wedding.templateId && tmpl.id === 'khmer-legacy')))
                                    ? "bg-rose-50/50 dark:bg-rose-500/10 border-rose-200 dark:border-rose-500/30 shadow-xl shadow-rose-500/5"
                                    : (!tmpl.comingSoon ? "bg-white dark:bg-white/5 border-slate-100 dark:border-white/5 hover:border-rose-100 dark:hover:border-white/10 hover:bg-slate-50 dark:hover:bg-white/[0.08] shadow-sm" : "")
                            )}
                        >
                            <div className="flex items-center gap-6">
                                ...
                                <div className="flex-1 space-y-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {tmpl.comingSoon && (
                                            <span className="px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9px] font-black uppercase tracking-wider">
                                                Coming Soon
                                            </span>
                                        )}
                                    </div>
