...
cloudinaryRouter.post('/sign', async (c) => {
    ...
    if (!apiSecret) {
        return c.json({ error: "Cloudinary API secret is not configured on the server" }, 500);
    }
...
cloudinaryRouter.post('/delete', async (c) => {
    ...
    try {
        ...
        if (user.role !== ROLES.PLATFORM_OWNER) {
            const files: any = await queryRaw('SELECT "weddingId" FROM "GalleryItem" WHERE "publicId" = $1 LIMIT 1', public_id);
            const file = files[0];

            let weddingId = (user as any).weddingId;
            if (!weddingId) {
                const rows: any = await queryRaw('SELECT id FROM "Wedding" WHERE "userId" = $1 LIMIT 1', user.userId);
                weddingId = rows[0]?.id;
            }
