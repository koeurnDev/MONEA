...
async function validateCSRF(request: NextRequest): Promise<boolean> {
    ...
    if (["POST", "PUT", "DELETE", "PATCH"].includes(request.method)) {
        ...
        const isPublicAuthRoute = ["/api/auth/signin", "/api/auth/signup", "/api/auth/send-otp", "/api/auth/verify", "/api/auth/csrf", "/api/auth/forgot-password", "/api/auth/reset-password", "/api/cloudinary/sign"].includes(request.nextUrl.pathname);
